...
proptest! {
    ...
    fn robust_prop_test(total_len in 1024u64..8192, chunk_len in 8u32..512) {
        ...
        let enc = RobustEncoder::new(buf, chunk_len as usize, EncoderType::Systematic, 0.2, None, 0.05);
...
proptest! {
    ...
    fn robust_prop_lossy_test(total_len in 1024u64..8192, chunk_len in 8u32..512) {
        ...
        for loss in losses {
            let enc = RobustEncoder::new(buf.clone(), chunk_len as usize, EncoderType::Systematic, 0.2, None, 0.05);
            let sw = Stopwatch::start_new();
...
fn run(enc: RobustEncoder, dec: &mut Decoder) -> Vec<u8> {
...
fn run_lossy(enc: RobustEncoder, dec: &mut Decoder, loss: f32) -> Vec<u8> {
    let mut out: Vec<u8> = vec![];
